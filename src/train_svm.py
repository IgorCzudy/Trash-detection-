...
def get_X_Y(data, verb=False, split=""):
    ...
    return X, Y, rectangles


def print_scores(X, Y, rectangles, classifier, split=""):
...
def train_and_test(verb=False):
    ...
    X_train, Y_train, rectangles_train = get_X_Y(train_data, verb, split="train")
    X_val, Y_val, rectangles_val = get_X_Y(val_data, verb, split="validation")
    X_test, Y_test, rectangles_test = get_X_Y(test_data, verb, split="test")
    ...
    scores_string = ""
    scores_string += print_scores(X_train, Y_train, rectangles_train, svm_classifier, split="train")
